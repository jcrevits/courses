defmodule ZoomWeb.Room.ShowLive do
  @moduledoc """
  A LiveView for creating and joining chat rooms.
  """

  use ZoomWeb, :live_view

  alias Zoom.ConnectedUser
  alias Zoom.Organizer
  ...
  def mount(%{"slug" => slug}, _session, socket) do
    ...
    Phoenix.PubSub.subscribe(Zoom.PubSub, "room:" <> slug)
    Phoenix.PubSub.subscribe(Zoom.PubSub, "room:" <> slug <> ":" <> user.uuid)
  ...
  defp send_direct_message(slug, to_user, event, payload) do
    ZoomWeb.Endpoint.broadcast_from(
      self(),
      "room:" <> slug <> ":" <> to_user,
      event,
      payload
